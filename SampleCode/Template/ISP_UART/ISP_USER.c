...
void EraseAP(unsigned int addr_start, unsigned int addr_end)
{
    ...
    for (; eraseLoop < (addr_start+addr_end); eraseLoop += FMC_FLASH_PAGE_SIZE) {
        #if 0   // debug
        if (flag)
        {
...
int ParseCmd(unsigned char *buffer, uint8_t len)
{
    ...
    if (lcmd == CMD_GET_FWVER) {
        ...
        // EraseAP(APROM_APPLICATION_START, (g_apromSize < g_dataFlashAddr) ? g_apromSize : g_dataFlashAddr);
